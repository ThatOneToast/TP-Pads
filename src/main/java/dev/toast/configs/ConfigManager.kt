 ...
 */
class ConfigManager {
    private val dataPath = Pads.getPlugin().dataFolder.absolutePath
    ...
    fun getConfig(
        ...
        val configFile: File =
            Pads.getPlugin().dataFolder.walk()
                .filter { it.isFile }
    ...
    fun createConfig(config: WLConfig) {
        when (config) {
            ...
            is WLBytesConfig -> {
                ...
                Pads.getPlugin().logger.log(Level.SEVERE, "Unknown config type Config: $config")
            }
        }
    }
